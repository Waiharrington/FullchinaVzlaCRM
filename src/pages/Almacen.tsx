  ...
  return (
    <div className="almacen-page management-workspace management-workspace--warehouse">
      ...
      {selectedItem && modalMode && createPortal(<div className={`almacen-modal-overlay ${closingItemModal ? 'closing' : ''}`} onClick={closeModal}><div className="almacen-action-modal" onClick={event => event.stopPropagation()}><button type="button" className="almacen-modal-close" onClick={closeModal} aria-label="Cerrar"><X size={16} /></button>
        {modalMode === 'view' && <><div className="almacen-modal-heading"><span><Eye size={18} /></span><div><small>Historial del insumo</small><h3>{selectedItem.name}</h3></div></div><div className="almacen-modal-stock"><small>Stock actual</small><strong>{selectedItem.quantity} {selectedItem.unit}</strong></div>{modalLoading ? <p className="almacen-modal-empty">Cargando movimientos…</p> : itemMovements.length === 0 ? <p className="almacen-modal-empty">Este insumo todavía no tiene movimientos.</p> : <div className="almacen-history-list">{itemMovements.map(movement => <div className="almacen-history-row" key={movement.id}><div className="almacen-history-main"><div className="almacen-history-title"><span className={`almacen-history-kind ${movement.quantity > 0 ? 'positive' : 'negative'}`}>{movement.quantity > 0 ? <Plus size={13} /> : <Minus size={13} />}</span><strong>{movement.movementType === 'purchase' ? 'Entrada' : movement.movementType === 'consumption' ? 'Salida' : 'Ajuste'}</strong></div><small>{movement.notes || movement.referenceType || 'Sin motivo'}</small><time>{new Date(movement.createdAt).toLocaleString('es-VE', { dateStyle: 'short', timeStyle: 'short' })}</time></div><b className={movement.quantity > 0 ? 'positive' : 'negative'}>{movement.quantity > 0 ? '+' : ''}{movement.quantity} {movement.unitSymbol}</b></div>)}</div>}</>}
