-- Full China Vzla: Binance y Zelle como metodos de pago reales.
...
ALTER TABLE fullchinavzla.payments
  ADD CONSTRAINT payments_method_check CHECK (method IN (
    'cash', 'mobile', 'card', 'transfer', 'binance', 'zelle', 'other'
  ));
...
BEGIN
  ...
  IF NEW.method IN ('mobile', 'card', 'transfer', 'binance', 'zelle')
...
BEGIN
  ...
  LOOP
    ...
    IF v_method NOT IN ('cash', 'mobile', 'card', 'transfer', 'binance', 'zelle', 'other') THEN
    ...
    IF v_method IN ('mobile', 'card', 'transfer', 'binance', 'zelle')
...
COMMENT ON COLUMN fullchinavzla.payments.reference_number IS
  'Referencia o comprobante obligatorio para pagos electronicos';
