-- =============================================================================
-- FULL CHINA VZLA - Costo de delivery por pedido
-- =============================================================================
-- El delivery se cotiza aparte (por WhatsApp) y se ingresa manualmente en Caja.
-- Se modela como un renglón de la orden (order_items) usando un producto oculto
-- "Delivery", con precio variable por pedido. Así el monto entra en total_amount,
-- pagos, cierres y reportes automáticamente (todos suman order_items).
-- =============================================================================
...
-- 1) Marcador de producto de delivery
ALTER TABLE fullchinavzla.sellable_products
  ADD COLUMN IF NOT EXISTS is_delivery boolean NOT NULL DEFAULT false;

-- 2) Producto oculto para el cargo de delivery (no aparece en la carta).
--    Si ya existe uno llamado 'Delivery', se marca; si no, se crea.
UPDATE fullchinavzla.sellable_products
   SET is_delivery = true
 WHERE name = 'Delivery';

INSERT INTO fullchinavzla.sellable_products (name, price, category, emoji, is_active, is_delivery)
SELECT 'Delivery', 0, 'extra', '🛵', false, true
WHERE NOT EXISTS (SELECT 1 FROM fullchinavzla.sellable_products WHERE is_delivery = true);

-- 3) Reemplazar fn_checkout_order añadiendo p_delivery_fee (renglón de delivery).
--    Se elimina la firma de 6 argumentos para evitar ambigüedad de overloads.
DROP FUNCTION IF EXISTS fullchinavzla.fn_checkout_order(jsonb, jsonb, numeric, text, text, text);
