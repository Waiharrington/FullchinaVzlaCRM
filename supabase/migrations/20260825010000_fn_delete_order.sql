...
-- items. Solo permite eliminar comandas no cobradas. El PIN de admin se valida
-- en el frontend; esta función asume que la autorización ya fue verificada.
...
BEGIN
  ...
  IF v_status IN ('paid', 'cancelled') THEN
    RAISE EXCEPTION 'No se puede eliminar una comanda ya cobrada o cancelada.';
  END IF;
  ...
  -- Eliminar modificadores, items y la comanda
...
REVOKE ALL ON FUNCTION fullchinavzla.fn_delete_order(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION fullchinavzla.fn_delete_order(uuid) TO authenticated;
